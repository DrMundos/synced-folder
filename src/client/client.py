...
def local_index():
    ...
    for dirpath, _, files in os.walk(SYNC_DIR):
        for fn in files:
            ...
            fp = os.path.join(SYNC_DIR, rel)
            idx[rel] = {"sha": sha256_of_file(fp), "mtime": os.stat(fp).st_mtime}
    return idx
...
def upload_file(path, base_sha):
    ...
    with open(fp, "rb") as f:
        data = f.read()
    meta = json.dumps({"path": path, "base_sha": base_sha}).encode()
...
def delete_on_server(path):
    ...
    log.info(f"Deleted remotely: {path}")
...
def save_state(files_list):
    try:
        json.dump({"files": sorted(files_list)}, open(STATE_FILE, "w"))
    except Exception:
        pass
...
def sync_once():
    ...
    # Detect deleted local files
    prev = set(load_state().get("files", []))
    curr = set(lidx.keys())
    for path in prev - curr:
        if path in sidx:
            try:
    ...
    for path, meta in sidx.items():
        fp = os.path.join(SYNC_DIR, path)
        if not os.path.exists(fp) or sha256_of_file(fp) != meta["sha"]:
            download_file(path)
